...
/**
 * Utility class that provides a simpler interface to access the location of a
 * {@link UnitView} or {@link ResourceView}.
 *
 * @author Ryan Tatton
 */
public class Location
{
     ...
     */
    public static double distBetween(ResourceView resource1,
                                     ResourceView resource2)
    {
     ...
     * @return Euclidean distance between a {@link UnitView} and
     * {@link ResourceView}.
     */
    ...
    public static boolean isOpen(StateView state, Location center, int radius
            , boolean checkCenter) throws IllegalArgumentException
    {
        if (radius < 1)
        ...
        if (!isOnBoard(state, center))
            throw new IllegalArgumentException(
                    "Location specified is not on the board.");
    ...
    {
        boolean isResourceAt = state.isResourceAt(
                location.getX(),
                location.getY()
        );
        boolean isUnitAt = state.isUnitAt(
                location.getX(),
                location.getY()
        );
    ...
    {
        return location.getX() <= state.getXExtent() &&
                location.getY() <= state.getYExtent();
    }
