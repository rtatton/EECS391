 ...
 * <p>
 * I've provided you with a simple Position class with some helper methods.
 * Use this for any place you need to track a location. If you need modify
 * the methods and add new ones. If you make changes add a note here about
...
public class Position
{
    ...
    public static Position locate(int x, int y)
    {
    ...
    {
        return new Position(direction.xComponent() + x,
                            direction.yComponent() + y);
    }
    ...
    public List<Position> getAdjacentPositions()
    {
        ...
        {
            if (inBounds(direction.xComponent(), direction.yComponent()))
    ...
    /**
     * Check if the position is within the map. Does not check if the
     * position is occupied
     *
     * @param xExtent X dimension size of the map (get this from the
     *                StateView object)
     * @param yExtent Y dimension size of the map (get this from the
     *                StateView object)
    ...
    {
        return Math.sqrt(Math.pow(getX() - position.getX(),
                                  2) + Math.pow(getY() - position.getY(), 2));
    }
    ...
    {
        return Math.max(Math.abs(getX() - position.getX()),
                        Math.abs(getY() - position.getY()));
    }

    /**
     * True if the specified position can be reached in one step. Does not
    ...
    public Direction getDirection(Position position)
    {
        ...
        System.err.println(
                "Position not adjacent. Could not determine direction");
        return null;
    }

    /**
     * Utility function. Allows you to check equality with pos1.equals(pos2)
     * instead of manually checking if x and y
     * are the same.
     *
